	...
	return (
		<div { ...blockProps }>
			<div className="sikada-auth-login-form-placeholder">
				...
				<p>
					{ __(
						'This block will display the user login form on the frontend.',
						'sikada-auth'
					) }
				</p>
				<div className="sikada-login-form-preview">
					...
					<div style={ { opacity: 0.6, marginTop: '10px' } }>
						<input
							type="text"
							placeholder={ __( 'Username', 'sikada-auth' ) }
							disabled
							style={ { width: '100%', marginBottom: '10px' } }
						/>
						<input
							type="password"
							placeholder={ __( 'Password', 'sikada-auth' ) }
							disabled
							style={ { width: '100%', marginBottom: '10px' } }
						/>
						<button disabled style={ { width: '100%' } }>
							{ __( 'Log In', 'sikada-auth' ) }
						</button>
					</div>
				</div>
			</div>
