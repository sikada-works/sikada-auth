(function () {
    ...
    function initResetForm() {
        ...
        forms.forEach(form => {
            form.addEventListener('submit', async (e) => {
                ...
                if (p1 !== p2) {
                    showMessage(form.querySelector('.sikada-form-messages'),
                        window.sikadaAuthData?.labels?.mismatch || 'Passwords do not match.', 'error');
                    return;
                }
    ...
    async function handleAjaxSubmit(form, action) {
        ...
        if (urlParams.has('login')) formData.append('login', urlParams.get('login'));
        ...
        submitButton.textContent = window.sikadaAuthData?.labels?.loading || 'Processing...';
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.data.redirect_url) {
                    ...
                    setTimeout(() => window.location.href = window.sikadaAuthData.loginUrl || '/wp-login.php', 1500);
                }
                form.reset();
            } else {
                showMessage(messagesContainer, data.data.message || 'Error occurred', 'error');
            }
        } catch (err) {
            showMessage(messagesContainer, 'Server connection failed.', 'error');
        } finally {
